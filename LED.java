 ...
 * 
 * Has a queue for LED events (either set or flash). Functions "on", "off", and
 * "flash" add the corresponding event to the queue as one of the colour codes.
 * A worker thread constantly processes the queue in the background while the
 * rest of the program is running. That way the program doesn't sit and wait
 * while it flashes the LED. If multiple flash commands come in at the same
 * time, they will happen one after the other in the order that they came in.
 *
 * PI4J includes functions like pulse() and blink() which should have done all
 * of this in a much simpler way, but they turned out to be unreliable in that
 * the LEDs wouldn't consistently flash at the same time. Trying to flash white
 * (RGB) would noticeably turn red on first, then green, then blue, instead of
 * all of them simultaneously.
 */
public class LED {

	// maximum value for an LED
	// in the future LED brightness could potentially be controllable by software
	// PWM
	...
	private int status; // last solid colour (or OFF) the LED was set to - returns to this after
											// flashing
	...
	/**
	 * Get the name of a colour from its code (used for console output)
	 * 
	 * @param colourCode Colour code
	...
	private void set(int colourCode) {
		switch (colourCode) {
			...
			case BLUE:
				...
				break;
			case YELLOW:
				set(PIN_MAX, PIN_MAX, 0, 0);
				break;
			case MAGENTA:
				set(PIN_MAX, 0, PIN_MAX, 0);
				break;
			case CYAN:
				set(0, PIN_MAX, PIN_MAX, 0);
				break;
			case WHITE:
				if (pins.size() > 3)
					set(0, 0, 0, PIN_MAX);
				else
					set(PIN_MAX, PIN_MAX, PIN_MAX, 0);
				break;
			case ALL:
				set(PIN_MAX, PIN_MAX, PIN_MAX, PIN_MAX);
				break;
			default:
				off();
				break;
