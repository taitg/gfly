 ...
 * module says it has a fix), and if data has been received more recently than
 * the timeout (currently 15sec), then the LED will be on.
 */
public class GPS {
	...
	public void init() {
		...
		try {
			...
			if (Config.gpsSource.equals("USB"))
				device = "/dev/ttyUSB0";
			config.device(device).baud(Baud._9600).dataBits(DataBits._8).parity(Parity.NONE).stopBits(StopBits._1)
	...
	public GPSData getLastComplete() {
		...
		if (result == null || result.getAltitude() <= 0)
			return null;
		return result;
	}
	...
	/**
	 * Get all GPS data (only keeps the last 15) - not currently in use by the
	 * program
	 * 
