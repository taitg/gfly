...
public class Struct {
    ...
    private byte[] packRaw_8b(byte val) {
        ...
        if (val >= 0) {
            ...
            int v2 = abs(val);
            v2 = (v2 ^ 0x7f) + 1; // invert bits and add 1
            v2 = v2 | (1 << 7);
            bx[0] = (byte) (v2 & 0xff);
    ...
    public byte[] pack(String fmt, long[] vals) throws Exception {
        ...
        if (len != vals.length)
            throw new Exception("format length and values aren't equal: " + len + " vs " + vals.length);
    ...
    private long unpackRaw_8b(byte[] val) {
    ...
    private long unpackRaw_u32b(byte[] val) {
        ...
        long x;
        x = (((long) (val[0] & 0xff)) << 24) | (((long) (val[1] & 0xff)) << 16) | (((long) (val[2] & 0xff)) << 8)
                | ((long) (val[3] & 0xff));
        return x;
    }
    ...
    public long[] unpack(String fmt, byte[] vals) throws Exception {
        ...
        for (int i = 0; i < fmt.length(); i++) {
            ...
            if ((i == 0) && ((c == '>') || (c == '<') || (c == '@') || (c == '!'))) {
                ...
                if ((c != '>') && (c != '<') && (c != '@') && (c != '!')) {
                    if (c == 'b') {
                        int read = bs.read(bChar);
                        bxx[p] = unpack_single_data(c, bChar);
                    }
                    if (c == 'h' || c == 'H') {
